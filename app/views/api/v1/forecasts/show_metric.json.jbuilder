...
json.current do
  ...
  json.temperature @forecast["current"]["temp_c"]
  json.wind do
  ...
  json.feels_like @forecast["current"]["feelslike_c"]
end
json.forecast @forecast["forecast"]["forecastday"] do |forecast|
  json.day forecast["date"]
  json.max forecast["day"]["maxtemp_c"]
  json.min forecast["day"]["mintemp_c"]
  ...
  json.hourly forecast["hour"] do |hour|
    json.time hour["time"]
    json.temperature hour["temp_c"]
    json.condition hour["condition"]["text"]
    json.feels_like hour["feelslike_c"]
  end
end
